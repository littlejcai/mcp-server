...
当前为 N0 最小 MVP：`list_skills` / `describe_skill` / `run_skill` 三个调度
工具 + 脚本型技能执行 + Streamable HTTP + Bearer 认证。Agent 型技能与
first-class 动态工具在 N1 落地（调用会得到明确的 "planned milestone N1" 错误）。
