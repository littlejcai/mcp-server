...
| **Agent 型技能**（`claude -p` 无头驱动，allowedTools 白名单） | ✅ 可用（N1） |
| **first-class 独立工具**（`first_class: true` 自动注册） | ✅ 可用（N1） |
...
| **N1（已完成）** | AgentRunner + first-class 工具；实弹 agent 链路通过；双端 parity 跑批待原 Python 环境 |
